...
class HttpFormatter(logging.Formatter):       
	...
	def formatMessage(self, record):
		...
		return result
formatter = HttpFormatter('{asctime} {levelname} {name} {message}')
...
class Node:
    ...
    def create_task(self, files, options={}, name=None, progress_callback=None, skip_post_processing=False, webhook=None, outputs=[], parallel_uploads=10, max_retries=5, retry_timeout=5):
        ...
        #print("PYODM POST /task/new/init time " +  str(datetime.datetime.now()))

        result = self.post('/task/new/init', data=e, headers={'Content-Type': e.content_type})
        #print("PYODM POST /task/new/init time after " +  str(datetime.datetime.now()))

	#print("************************* PYODM post /task/new/init")
            ...
            def worker():
                while True:
                    ...
                    try:
                        ...
                        e = MultipartEncoder(fields=fields)
			#print("PYODM POST /task/new/upload time " +  str(datetime.datetime.now()))
                        result = self.post('/task/new/upload/{}'.format(uuid), data=e, headers={'Content-Type': e.content_type})
                        #print("PYODM POST /task/new/upload time after " +  str(datetime.datetime.now()))

			#print("************************* PYODM post /task/new/upload")
       			#print(result.elapsed.total_seconds())
...
class Task:
    ...
    def download_zip(self, destination, progress_callback=None, parallel_downloads=16, parallel_chunks_size=10):
        ...
        try:
            print("*****PYODM download stream of /task/ download/ all.zip time before get " + str(datetime.datetime.now()))
            ...
            #print("*****PYODM zip_path from /task/download /all zip byte size " + str(os.stat(zip_path).st_size)) 
            # Keep track of download progress (if possible)
            content_length = download_stream.headers.get('content-length')
	    print("******PYODM zip file content length " + str(content_length))  
            total_length = int(content_length) if content_length is not None else None
            downloaded = 0
            chunk_size = int(parallel_chunks_size * 1024 * 1024)
	    print("*******PYODM chuck size *** " + str(chunk_size))
            use_fallback = False
            ...
            if accept_ranges is not None and accept_ranges.lower() == 'bytes' and total_length is not None and total_length > chunk_size and parallel_downloads > 1:
                ...
                def worker():
                    while True:
                        ...
                        try:
                            # Download chunk
			    print("****PYODM downloading chunk stream **** Time now " + str(datetime.datetime.now())+ " uuid " + str(self.uuid) + " part num" + str(part_num) );
                            res = self.get('/task/{}/download/all.zip'.format(self.uuid), stream=True, headers={'Range': 'bytes=%s-%s' % bytes_range})
                            print("*****PYODM res response got back time: " + str(datetime.datetime.now()) + " uuid "+ str(self.uuid) + " part num " + str(part_num));
